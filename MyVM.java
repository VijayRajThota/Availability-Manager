	...
	synchronized public Boolean pingVM() throws IOException
    {
		...
		if (hostIp != null) {
			if (pingResult.contains("100% loss"))
			{
				...
				System.out.println("VM is not responding!");
				reach = false;
				valueSet = false;
			}
			else
			{
				System.out.println("VM is live");
				System.out.println("Pinging on: " + hostIp);
				reach = true;
				notifyAll();
			}
		}
		else{
			System.out.println("VM IP is:"+hostIp);
			reach=false;
			valueSet=false;
		}
		return reach;
